...
$MSGraph = Read-Host "`nPlease enter the software name."

try 
{
    # Replace occurrences of "MSGraph" in all files
    Write-Host "`nReplacing occurrences of 'MSGraph' with "$MSGraph" in all files.`n"
    ...
    Get-ChildItem -File -Recurse | ForEach-Object {
        ...
        try
        {
            (Get-Content $_.FullName) -replace 'MSGraph', $MSGraph | Set-Content $_.FullName
        } 
        catch 
        {
    ...
    Write-Host "`nRenaming files and directories using "$MSGraph":`n"
    ...
    # Get all files and directories containing "MSGraph" recursively
    Get-ChildItem -Recurse -Directory | ForEach-Object {
        $dirpath = $_.FullName
        $dirname = Split-Path  $dirpath -Leaf
        if ($dirname.Contains("MSGraph"))
        {
            $stack.Push($dirpath)
        ...
        foreach ($file in [Directory]::EnumerateFiles($dirpath)) 
        {
            ...
            if ($filename.Contains('MSGraph') -and -not $allPaths.Contains($file))
            {
                $stack.Push($file)
                $allPaths.Add($file)
            }
    ...
    Get-ChildItem -File | ForEach-Object {
        if ($_.FullName.Contains("MSGraph")) {
    ...
    while ($stack.Count) {
        ...
        if($filename.Contains('MSGraph') -and $pathExists)
        {
            $newName = $filename.Replace('MSGraph', $MSGraph)
